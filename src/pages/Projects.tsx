      ...
      <section ref={sectionRef} className="py-12 bg-background relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <motion.div
            ...
            className="grid grid-cols-3 sm:flex sm:flex-wrap sm:justify-center gap-2 sm:gap-3 md:gap-4 mb-16"
          >
            {categories.map((category) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`flex items-center justify-center gap-1 sm:gap-2 px-2 py-2 sm:px-4 sm:py-2 md:px-8 md:py-4 rounded-lg sm:rounded-xl font-semibold text-[10px] sm:text-xs md:text-sm lg:text-lg transition-all ${
                  activeCategory === category.id
                    ? 'gradient-gold text-foreground shadow-gold'
                    : 'bg-card border-2 border-border text-foreground hover:border-accent'
                }`}
              >
                <category.icon className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 flex-shrink-0" />
                <span className="text-center break-words leading-tight">{category.name}</span>
              </motion.button>
            ))}
          </motion.div>
